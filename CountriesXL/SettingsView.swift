...
private struct PrivacySettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                Toggle("Enable Push Notifications", isOn: $settings.draft.pushNotificationsEnabled)
                Toggle("Enable Location", isOn: $settings.draft.locationEnabled)
                ...
                Toggle("Disable Ads", isOn: $settings.draft.adsDisabled)
                Toggle("Disable Video Ads", isOn: $settings.draft.disableVideoAds)
                    .disabled(settings.draft.adsDisabled)
                    .help("If all ads are disabled, this is implied.")
            }

            Section {
                Toggle("Biometric Lock (Touch ID)", isOn: Binding(
                    get: { biometricLockEnabled },
                    set: { newValue in
                        if newValue {
                            authenticateBiometricLock(enable: true)
                        } else {
                            authenticateBiometricLock(enable: false)
                        }
                    }
                ))

                Toggle("Send Anonymous Usage Data", isOn: $settings.draft.sendAnonymousUsageData)

                Button("Clear Cache/Data", role: .destructive) {
                    // Clear cache and data functionality
                }
                Text("Removes temporary files and cached data. You may be signed out of services.")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            ...
            Section(header: Text("App Permissions")) {
                Toggle("Notifications", isOn: .constant(true))
                    .disabled(true)
                Toggle("Files Access", isOn: .constant(false))
                    .disabled(true)
                Toggle("Location Access", isOn: .constant(true))
                    .disabled(true)
            }
        }
        .formStyle(.grouped)
        ...
        .alert(item: $biometricAlert) { alert in
            Alert(title: Text("Biometric Authentication"), message: Text(alert.message), dismissButton: .default(Text("OK")))
        }
    }
    ...
    private func authenticateBiometricLock(enable: Bool) {
        ...
        if context.canEvaluatePolicy(policy, error: &error) {
            let reason = enable ? "Enable Biometric Lock" : "Disable Biometric Lock"
            context.evaluatePolicy(policy, localizedReason: reason) { success, _ in
                DispatchQueue.main.async {
                    if success {
                        biometricLockEnabled = enable
                    } else {
                        biometricLockEnabled = !enable
                        biometricAlert = BiometricAlert(message: "Authentication failed or was cancelled.")
                    }
                }
            }
        } else {
            DispatchQueue.main.async {
                biometricAlert = BiometricAlert(message: "Authentication is not available on this device.")
                biometricLockEnabled = !enable
            }
        }
    }
...
private struct FeedbackFormView: View {
                            ...
                            TextEditor(text: $message)
                                ...
                                .overlay(
                                    RoundedRectangle(cornerRadius: 6)
                                        .stroke(messageError != nil ? Color.red : Color.gray.opacity(0.3), lineWidth: 1)
                                )
    ...
    private func isFormValid() -> Bool {
        ...
        let validEmail = !trimmed.isEmpty && isValidEmail(trimmed)
        let validMessage = !message.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    ...
    private func sendFeedback() {
        ...
        let feedbackEmail = "feedback@example.com"
...
private struct SettingsDiagnosticsView: View {
    ...
    @State private var showingFolderInFinder = false
    @State private var showFolderPicker = false
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            Form {
                Section(header: Text("Download Folder")) {
                    ...
                    Text(url?.path ?? "Not set").font(.callout).foregroundColor(.secondary)
                    HStack {
                        Button("Choose…") { showFolderPicker = true }
                        Button("Test Access") { testAccess() }
                        Button("Reveal in Finder") { revealInFinder() }.disabled(url == nil)
                        Button("Use Default Downloads Folder") { setDefaultDownloads() }.disabled(url != nil)
                    }
                    .onAppear { refreshStatus() }
                    .onChange(of: settings.draft.downloadPath) { refreshStatus() }
                    Button("Clear Bookmark") {
                        showClearConfirm = true
                    }
                    .buttonStyle(.bordered)
                    if !accessResult.isEmpty {
                        Text(accessResult).font(.caption).foregroundColor(.secondary)
                    }
                }
                Section(header: Text("Login Item")) {
                    Text("Status: \(settings.loginItemStatusString())")
                    HStack {
                        Button("Register") { try? settings.setLoginItemEnabled(true) }
                        Button("Unregister") { try? settings.setLoginItemEnabled(false) }
                    }
    ...
    private func testAccess() {
        ...
        settings.endAccessingDownloadFolder(url)
    }
    ...
    private func refreshStatus() {
        ...
        if url != nil {
            accessOK = true
            accessResult = "Access OK: \(url!.path)"
        } else {
            accessOK = false
            accessResult = "No folder configured or access failed."
        }
...
// Added extension for notification name
extension Notification.Name {
    static let openSettingsDiagnostics = Notification.Name("OpenSettingsDiagnostics")
}
